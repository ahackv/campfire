...
const AUDIO_ASSETS = {
  victory: ["./assets/sfx/nothing-beats-a-jet2-holiday.mp3", "./assets/nothing-beats-a-jet2-holiday.mp3"],
  chaos: [
    "./assets/sfx/wait-wait-wait-what-the-hell.mp3",
    "./assets/wait-wait-wait-what-the-hell.mp3",
    "./assets/wait-wait-what-the-hell.mp3",
  ],
  fail: ["./assets/sfx/spongebob-fail.mp3", "./assets/spongebob-fail.mp3"],
};
const sfxCache = {};
let sfxEnabled = true;

function createAudioFromCandidates(candidates = [], volume = 0.7) {
...
function createSfx(candidates = []) {
  return createAudioFromCandidates(candidates, 0.7);
}
...
function stopBgMusicPlayers() {
  bgMusicPlayers.forEach((player) => {
    player.pause();
    try {
...
function playBgMusicTrack(index, restart = false) {
  if (!bgMusicPlayers.length) return false;

  const normalized = ((index % bgMusicPlayers.length) + bgMusicPlayers.length) % bgMusicPlayers.length;
  bgMusicIndex = normalized;

  bgMusicPlayers.forEach((player, i) => {
  ...
  const player = bgMusicPlayers[normalized];
  const track = BG_MUSIC_TRACKS[normalized] || { label: `Track ${normalized + 1}` };
  if (restart) {
    try {
      player.currentTime = 0;
    } catch (_err) {
  ...
  const onStarted = () => {
    usingFileMusic = true;
    stopSynthMusic();
    el.botMood.textContent = `Now playing: ${track.label}`;
  };
...
function ensureBgMusicPlayers() {
  if (bgMusicPlayers.length) return bgMusicPlayers;
  bgMusicPlayers = BG_MUSIC_TRACKS.map((track, index) => {
    const player = createAudioFromCandidates(track.sources, 0.45);
    if (!player) return null;
    player.loop = false;
    player.addEventListener("ended", () => {
      if (!musicOn || !usingFileMusic) return;
      const next = (index + 1) % BG_MUSIC_TRACKS.length;
      playBgMusicTrack(next, true);
    });
    return player;
  }).filter(Boolean);
